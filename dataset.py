...
class BaseDataset(Dataset):
    def __init__(self, model_size='base', phase='train'):
        super(BaseDataset, self).__init__()
        self.emotion_dict  = {'angry': 0, 'anger': 0,
        'disgust': 1,
        'fear': 2,
        ...
        self.speaker_dict = {'A': 0, 'B': 1}
        self.tokenizer = RobertaTokenizer.from_pretrained('roberta-'+model_size)
        data_path = './dataset/dailydialog_'+phase+'.json'
        processed_data_path = './dataset1/dailydialog_processed_1'+phase+'.json'
        ...
        if os.path.exists(processed_data_path):
            ...
            for doc_id, content in data.items():
                ...
                evi = []
                # act_lbl = []
                self.conv_len.append(len(content))
                for utt in content:
                    u = utt['utterance']
                    encoded_output = self.tokenizer(u)
                    tid = encoded_output.input_ids
                    ...
                    if 'expanded emotion cause evidence' in utt:
                        ev = utt['expanded emotion cause evidence']
                    else:
                        ev = []
                    utter.append(u)
                    emo.append(self.emotion_dict[e])
            ...
            #         # act_lbl.append(self.act_mapping[a])
                ...
                # self.act_label.append(act_lbl)
            # to_be_saved_data = [self.utterance, self.token_ids, self.attention_mask, self.emotion,
                                # self.adj_index, self.evidence, self.mask, self.conv_len]
            # pickle.dump(to_be_saved_data, open(processed_data_path, 'wb'))
    ...
    def __getitem__(self, item):
        ...
        return utter, token_id, att_mask, label, adj_idx, evid, msk, clen,1
...
def collate_fn(data):
    ...
    return token_ids_1, attention_mask_1, clen, mask, adj_index, label, ece_pair,1
 
def get_dataloaders(model_size, batch_size, valid_shuffle):


    train_set = BaseDataset(model_size, 'train')
    dev_set = BaseDataset(model_size, 'valid')
    dd_set = BaseDataset(model_size, 'test')
    ie_set = BaseDataset(model_size,'IE')
